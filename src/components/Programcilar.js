...
export default function Programcilar() {
  const [iyilerListesi,setiyilerListesi] =useState(enIyilerListesi)
  const [programci,setProgramci]=useState(null)
  ...
  const secilenProgramci = iyilerListesi.find((item)=>{
return item.id==programci
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programci ==null ?'royalblue':"gold" // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          " */
          iyilerListesi.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setProgramci(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
